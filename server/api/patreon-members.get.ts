...
export default defineEventHandler(async () => {
  ...
  do {
    ...
    try {
      ...
      const statusMessage = error?.data?.errors?.[0]?.detail ?? error?.message ?? 'Failed to fetch Patreon members.'
    ...
    for (const member of response.data ?? []) {
      if (member.type !== 'member' || member.attributes?.patron_status !== 'active_patron') {
        continue
      }
